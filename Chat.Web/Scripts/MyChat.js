...
$(function () {
    ...
    chatHub.client.onRoomDeleted = function (message) {
        ...
        // Join to the first room in list (UPDATE: HAPPENS ONLY IF EXISTS)
        if ($("#room-list").children().length == 0) {
            $(".select-room").show();
            $(".chat-input").hide();
            $(".no-messages").hide();
        } else {
    ...
    Model.prototype = {
        ...
        joinRoom: function (room) {
            ...
            if (room) {
                ...
                $("input#iRoom").val("Home");
                $("#joinedRoom").html("<b>" + "Home" + "</b>");
            }
            $('#room-list a').removeClass('active');
        ...
        messageHistory: function (fromUserId = null, toUserId = null) {
            ...
            chatHub.server.getMessageHistory(roomId, fromUserId, toUserId).done(function (result) {
                ...
                if ($("#message-list").children().length == 0 && !$(".select-room").is(':visible')) {
                    $(".no-messages").show();
                } else {
                    $(".no-messages").hide();
                }
